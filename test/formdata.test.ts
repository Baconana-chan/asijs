...
describe("FormData Parsing", () => {
  ...
  describe("FileSchema", () => {
    ...
    it("should validate file max size", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors?.some((e) => e.message.includes("too large"))).toBe(
        true,
      );
    });
    ...
    it("should validate file MIME type", async () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.errors?.some((e) => e.message.includes("Invalid file type")),
      ).toBe(true);
    });
    ...
    it("should validate min file size", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors?.some((e) => e.message.includes("too small"))).toBe(
        true,
      );
    });
  });
  ...
  describe("Integration with Asi", () => {
    ...
    it("should access file via ctx.file()", async () => {
      ...
      const file = new File(["hello world content"], "doc.pdf", {
