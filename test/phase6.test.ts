...
describe("Phase 6 Features", () => {
  describe("Lifecycle Manager", () => {
    ...
    it("should register shutdown handlers", async () => {
      const manager = new LifecycleManager({ handleSignals: false, verbose: false });
      let called = false;
  ...
  describe("Response Caching", () => {
    ...
    it("should build Cache-Control header", () => {
      ...
      expect(buildCacheControl({ ttl: "5m", private: true })).toBe("private, max-age=300");
      expect(buildCacheControl({ noStore: true })).toBe("no-store");
      expect(buildCacheControl({ ttl: "1h", immutable: true })).toBe("public, max-age=3600, immutable");
    });
  ...
  describe("Dev Mode", () => {
    ...
    it("should provide routes API", async () => {
      ...
      const res = await app.handle(new Request("http://localhost/__dev/routes"));
      expect(res.status).toBe(200);
  ...
  describe("Auto Port", () => {
    it("should find next available port when current is in use", async () => {
      const app1 = new Asi({ development: true, silent: true, startupBanner: false });
      app1.get("/", () => "app1");
      
      const app2 = new Asi({ development: true, silent: true, startupBanner: false });
      app2.get("/", () => "app2");
    ...
    it("should throw error when autoPort is disabled", () => {
      const app1 = new Asi({ autoPort: false, silent: true, startupBanner: false });
      app1.get("/", () => "app1");
      
      const app2 = new Asi({ autoPort: false, silent: true, startupBanner: false });
      app2.get("/", () => "app2");
