import type { Route, RouteMatch, RouteMethod, Handler, Middleware } from "./types";
...
interface TrieNode {
  handlers: Map<RouteMethod, { handler: Handler; middlewares: Middleware[]; path: string }>;
  children: Map<string, TrieNode>;
  paramChild: { name: string; node: TrieNode } | null;
  wildcardHandler: { handler: Handler; middlewares: Middleware[]; path: string } | null;
}
...
export class Router {
   ...
   * Добавить роут
   */
  add(method: RouteMethod, path: string, handler: Handler, middlewares: Middleware[] = []): void {
  ...
  private findNode(
    ...
    if (staticChild) {
      const result = this.findNode(staticChild, segments, index + 1, params, method);
    ...
    if (node.paramChild) {
      ...
      const result = this.findNode(node.paramChild.node, segments, index + 1, params, method);
      if (result) {
        return result;
      }
